...
#メリット：Dataframeで情報処理できる、すでに並び替えた他のデータを合併できる、rtの入力制限ができるかも
#デメリット：コーディングが難しい、データないところをどうするか
#rtの入力制限する
#縦に並べることと、横に並べることをスイッチする
#########################################################
...
for data in data_dic:
    ...
    file.close()
    rt_of_max_area = 0
    if ~df2.isnull().all(axis=1).all():
        rt_of_max_area = round(df2["保持時間"][df2["面積"]==df2["面積"].max()].values[0],3)
    print("サンプル名：",df1.loc["サンプル名"].values[0])
    print("面積最大の保持時間：{} min".format(rt_of_max_area))
    pyperclip.copy(rt_of_max_area)
    std_rt = input_rt()
    print()
...
for data in data_dic:
    ...
    for d in os.listdir():
        if d[:-4]==data[:-4]:
            ...
            sample_name = str(df1.loc["サンプル名"].values[0])
            if "/" in sample_name:
